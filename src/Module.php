 ...
 *                 'columns' => ['title', 'title_ascii'], //names of multilingual fields
...
class Module extends \yii\base\Module
{
    ...
    /**
     * Query param used to enable disable translation
     */
    const QP_SWITCH_TRANSLATE = '_qst';

    /**
     * @inheritdoc
     */
    public $controllerNamespace = 'dlds\translatemanager\controllers';
    ...
    /**
     * @var array migrator rules specifies where to find translations to be migrated
     */
    public $migratorRules = [];

    /**
     * @var int max entries in one batch
     */
    public $migratorLimit = 100;
     ...
     *          'columns' => ['title', 'title_ascii'] // fields to check.
     *      ],
     *      [
     *          'connection' => 'db',                           // connection identifier.
     ...
     */
    public $languageTable = '{{%core_language}}';
    ...
    {
        if ($this->rbacPermission && !\Yii::$app->user->can($this->rbacPermission)) {
            return false;
        }
    ...
    public function checkTopAccess()
    {
        if ($this->rbacTopPermission && !\Yii::$app->user->can($this->rbacTopPermission)) {
            return false;
        }
