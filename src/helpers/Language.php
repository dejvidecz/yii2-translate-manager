...
namespace dlds\translatemanager\helpers;
...
class Language
{
     ...
     *      ],
     * ];
     * $result = \dlds\translatemanager\helpers\Language::a($array, $params);
     * ~~~
     *
     * The result:
     *
     ...
     * 'modules' => [
     *      'translatemanager' => [
     *          'class' => 'dlds\translatemanager\Module',
     *          'tables' => [
     *              [
     *                  'connection' => 'db',
     *                  'table' => 'language',
    ...
    {
        return Yii::$app->session->has(\dlds\translatemanager\Module::SESSION_KEY_ENABLE_TRANSLATE);
    }
    ...
    {
        $languageSources = \dlds\translatemanager\models\LanguageSource::find()->where(['category' => $category])->all();
        ...
        if (empty($messages[$message])) {
            $languageSource = new \dlds\translatemanager\models\LanguageSource();
    ...
    {
        $languageSources = \dlds\translatemanager\models\LanguageSource::find()->select('category')->distinct()->all();
